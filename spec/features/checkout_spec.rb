...
describe 'Checkout', type: :feature do
  ...
  shared_context 'purchases product with item included' do
    before do
      add_product_to_cart
  ...
  context 'backend order shipments UI', js: true do
    context 'ordering only the product assembly' do
      include_context 'purchases product with item included'

      it 'views items bundled as well' do
    ...
    context 'ordering assembly and the item as individual sale' do
      before do
      ...
      it 'views items bundled and not' do
        visit spree.admin_orders_path
        click_on Spree::Order.last.number
