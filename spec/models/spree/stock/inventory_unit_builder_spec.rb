  ...
  module Stock
    describe InventoryUnitBuilder, type: :model do
      ...
      context 'order shares variant as individual and within bundle' do
        include_context(
          'product is ordered as individual and within a bundle'
        ) do
          ...
          describe '#units' do
            it 'returns an inventory unit for each item of each quantity ' \
              "for the order's line items" do
              units = subject.units
              expect(units.count).to eq 4
              expect(units[0].line_item.quantity)
                .to eq order.line_items.first.quantity
              expect(units[0].line_item.quantity)
                .to eq bundle_item_quantity

              line_item = order.line_items.first

              expect(units.map(&:variant)).to match_array line_item.items
            end
